//this script contains cutscene 2's script
function script_cutscene_2()
{
	myText[0][ENG] = "{Ow... Where...\n Where am I?{";
	myText[1][ENG] = "{I see the anesthetic is\n wearing off.{";
	myText[2][ENG] = "{Who are you?!{";
	myText[3][ENG] = "{Shhh. Don't make so\n much noise...\n I've got a favor to ask\n of you.{";
	myText[4][ENG] = "{Here. Take this and \n escape.{\n{What is this...\n this statue?{";
	myText[5][ENG] = "{There's no time to\n explain. Hurry.{";
	myText[6][ENG] = "{Hey, wait. Who are you?";
	myText[7][ENG] = "{Didn't you try to kill \n me?{";
	myText[8][ENG] = "{Someone's coming.\n Be careful.{";
	
	myText[0][JPN] = "「う…… ここは…とこた…」";
	myText[1][JPN] = "「ますいか きれたようね。」";
	myText[2][JPN] = "「おまえは!」";
	myText[3][JPN] = "「シッ 大きなこえをたさないて\n あなたにたのみかあるの。」";
	myText[4][JPN] = "「これをもってにけて。」\n「句た、この像は…」";
	myText[5][JPN] = "「せつめいしてるひまはないの。さあ、はやく。」";
	myText[6][JPN] = "「待ってくれ、きみ拘者た?」";
	myText[7][JPN] = "「おれをころそうとしていたんしゃないのか?」";
	myText[8][JPN] = "「たれかくるわ。気をつけて。」";
	
	myText[0][ESP] = "{Ug... Donde...\n ¿Donde estoy?{";
	myText[1][ESP] = "{Por fin te despertaste.\n Esa anestesia\n funcionó bien.{";
	myText[2][ESP] = "{¡¿Quién eres?!{";
	myText[3][ESP] = "{Shhh. No hagas\n tanto ruido...\n Tengo un favor para ti.{";
	myText[4][ESP] = "{Toma esto y escapa \nde aqui.{\n{¿Qué es esto...?\n ¿Una estatua?{";
	myText[5][ESP] = "{No esperes una\n explicación. Solo corre.{";
	myText[6][ESP] = "{¡Espera un momento!\n ¿Quién eres? ";
	myText[7][ESP] = "{¿No intentaste\n matarme?{";
	myText[8][ESP] = "{Alguien se acerca.\n Ten cuidado.{";
	
	myText[0][ITA] = "{Ough ... Dove...\n Dove mi trovo?{";
	myText[1][ITA] = "{Vedo che l'effetto\n dell'anestetico\n sta svanendo.{";
	myText[2][ITA] = "{E tu chi sei?!{";
	myText[3][ITA] = "{Shhh. Non fare\n troppo rumore...\n Ho un favore \n da chiederti.{";
	myText[4][ITA] = "{Prendi. Tieni questa e\n scappa.{\n{Cos'è questa...\n questa statua?{";
	myText[5][ITA] = "{Non c'è tempo per le\n spiegazioni. Sbrigati.{";
	myText[6][ITA] = "{Ehi, aspetta. E tu chi\n saresti?";
	myText[7][ITA] = "{Non avevi provato ad\n uccidermi?{";
	myText[8][ITA] = "{Arriva qualcuno.\n Fai attenzione.{";
	
	myText[0][ROM] = "{Au... Unde...\n Unde sunt?{";
	myText[1][ROM] = "{Văd că efectul\n anestezicului\n se termină.{";
	myText[2][ROM] = "{Cine ești?{";
	myText[3][ROM] = "{ȘȘȘ. Nu face gălagie...\n Am o dorință să-ți cer.{";
	myText[4][ROM] = "{Aici. Ia asta și scapă.{\n Ce este asta...\n această statuie?{";
	myText[5][ROM] = "{Nu este timp de explicat.\n Grăbește-te.{";
	myText[6][ROM] = "{Hei, așteaptă. Cine ești?{";
	myText[7][ROM] = "{Nu încercai să\n mă omori?{";
	myText[8][ROM] = "{Vine cineva.\n Ai grijă.{";
	
	myText[0][420] = "         { {";
	myText[1][420] = "";
	myText[2][420] = "";
	myText[3][420] = "          {   {";
	myText[4][420] = "        {";
	myText[5][420] = "";
	myText[6][420] = "           {";
	myText[7][420] = "              { ";
	myText[8][420] = " {";
}