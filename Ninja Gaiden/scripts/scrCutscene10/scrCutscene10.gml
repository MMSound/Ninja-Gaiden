//this script contains cutscene 10's script
function script_cutscene_10()
{
	myText[0][ENG] = "{I am Malth.\n You will never get past\n me.{";
	myText[1][ENG] = "{Then I must defeat you!{";
	myText[2][ENG] = "{You are as bold as your\n father. But he is a\n much better swordsman.{";
	myText[3][ENG] = "{You know my father?{";
	myText[4][ENG] = "{Come and fight, young\n Hayabusa!{";
	
	myText[0][JPN] = "「このさきは このマルスか行かせん!」";
	myText[1][JPN] = "「ならは、キサマをたおす!」";
	myText[2][JPN] = "「フッ 気のつよいところか\n おまえのおやしにそっくりたな。\n もっとも、剣の うては\n おまえのおやしのほうかうえたったかな。」";
	myText[3][JPN] = "「キサマ!父さんをしっているのか?」";
	myText[4][JPN] = "「こい! リュウ.ハヤフサ!」";
	
	myText[0][ESP] = "{Yo soy Malus Sangriento.\n Yo jamás te dejare pasar.{";
	myText[1][ESP] = "{¡Entonces tendré que\n derrotarte!{";
	myText[2][ESP] = "{Eres igual de idiota que\n tú padre. Pero ya puedo\n ver que él fue un mejor\n espadachín.{";
	myText[3][ESP] = "{¿Conoces a mi padre?{";
	myText[4][ESP] = "{¡Cállate y pelea, joven \n Hayabusa!{";
	
	myText[0][ITA] = "{Io sono Malth.\n Non riuscirai mai a\n superarmi.{";
	myText[1][ITA] = "{Allora dovrò\n sconfiggerti!{";
	myText[2][ITA] = "{Sei audace come tuo\n padre. Ma lui è\n uno spadaccino migliore.{";
	myText[3][ITA] = "{Conosci mio padre?{";
	myText[4][ITA] = "{Vieni a combattere,\n giovane Hayabusa!{";
	
	myText[0][ROM] = "{Eu sunt Malth.\n Nu vei trece\n niciodată de mine.{";
	myText[1][ROM] = "{Atunci trebuie să\n te înving!{";
	myText[2][ROM] = "{Ești la fel de\n îndrăzneț ca tatăl tău.\n Dar el este un\n spadasin mai bun.{";
	myText[3][ROM] = "{Tu îmi cunoști tatăl?{";
	myText[4][ROM] = "{Vino și luptă, tinere\n Hayabusa!{";
	
	myText[0][FRE] = "{Mon nom est Malth.\n Tu ne me passeras jamais\n sur le corps.{";
	myText[1][FRE] = "{Donc je vais te battre!{";
	myText[2][FRE] = "{Tu es aussi fougueux\n que ton père. Il est\n cependant bien meilleur\n escrimeur.{";
	myText[3][FRE] = "{Tu connais mon père?{";
	myText[4][FRE] = "{Viens m'affronter,\n jeune Hayabusa!{";
	
	myText[0][420] = "            {";
	myText[1][420] = "   {";
	myText[2][420] = "            {\n       {     {  \n          { \n       {   {  {  {";
	myText[3][420] = "";
	myText[4][420] = "           {";
}