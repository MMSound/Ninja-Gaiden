//this script contains cutscene 11's script
function script_cutscene_11()
{
	myText[0][ENG] = "{You killed my father.{";
	myText[1][ENG] = "{Killed? It is true\n that we fought.\n But your father is\n alive.{";
	myText[2][ENG] = "{Liar!{";
	myText[3][ENG] = "{No, it is not a lie.\n If you go on,\n you will see.{";
	myText[4][ENG] = "{But it will be the last\n thing you see.{";
	myText[5][ENG] = "{...{";
	
	myText[0][JPN] = "「父さんをころしたのはキサマたな?」";
	myText[1][JPN] = "「ころす…? フッ 決闘をしたのはたしかた…\n たか、おまえのおやしは… また生きている… 」";
	myText[2][JPN] = "「なに! キサマ、今さらそんなテタラメを!」";
	myText[3][JPN] = "「テタラメてはない… ここからさきにすすむならは…";
	myText[4][JPN] = " おまえはこの世の… 地こくをしることになる…";
	myText[5][JPN] = "「… 」";
	
	myText[0][ESP] = "{Mataste a mi padre.{";
	myText[1][ESP] = "{¿Matarlo? Es cierto que\n tuvimos un duelo, pero\n eso no quiere decir que\n esté muerto.{";
	myText[2][ESP] = "{¡Mientes!{";
	myText[3][ESP] = "{No miento.\n El camino que sigues,\n joven Hayabusa, te\n llevará hacia él...{";
	myText[4][ESP] = "{...pero puede que ya\n no sea la persona\n que solías conocer...{";
	myText[5][ESP] = "{...{";
	
	myText[0][ITA] = "{Hai ucciso mio padre.{";
	myText[1][ITA] = "{Ucciso? È vero,\n abbiamo combattuto.\n Ma tuo padre è\n ancora vivo.{";
	myText[2][ITA] = "{Bugiardo!{";
	myText[3][ITA] = "{No, non sto mentendo.\n Se vai avanti,\n lo capirai.{";
	myText[4][ITA] = "{Ma sarà l'ultima cosa\n che vedrai.{";
	myText[5][ITA] = "{...{";
	
	myText[0][ROM] = "{Mi-ai ucis tatăl.{";
	myText[1][ROM] = "{Ucis? Este adevărat că\n ne-am luptat.\n Dar tatăl tău este\n în viață.{";
	myText[2][ROM] = "{Mincinosule!{";
	myText[3][ROM] = "{Nu, aceasta nu\n e minciună.\n Dacă tu continui,\n o să vezi.{";
	myText[4][ROM] = "{Dar o să fie\n ultimul lucru\n pe care îl vei vedea.{";
	myText[5][ROM] = "{...{";
	
	myText[0][FRE] = "{Tu as tué mon père.{";
	myText[1][FRE] = "{Tué? Il est vrai que\n nous avons combattu.\n Mais ton père est toujours\n parmi les vivants.{";
	myText[2][FRE] = "{Menteur!{";
	myText[3][FRE] = "{Non, cela ne saurait\n être un mensonge. Tu le\n verras par toi-même si tu\n continues sur ta voie.{";
	myText[4][FRE] = "{Mais cela sera la dernière\n que tu verras de tes\n yeux.{";
	myText[5][FRE] = "{...{";
	
	myText[0][420] = "              {";
	myText[1][420] = "                    { \n {{       {    {";
	myText[2][420] = "               {";
	myText[3][420] = " {   {                 {";
	myText[4][420] = "            {";
	myText[5][420] = "";
}